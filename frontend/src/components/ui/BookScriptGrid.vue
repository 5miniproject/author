    ...
    <v-container>
        ...
        <div class="panel">
            <div class="gs-bundle-of-buttons" style="max-height:10vh;">
                ...
                <v-btn :disabled="!selectedRow" style="margin-left: 5px;" @click="scriptPublishRequestDialog = true" class="contrast-primary-text" small color="primary" :disabled="!hasRole('Author')">
                    <v-icon small>mdi-minus-circle-outline</v-icon>도서 출간 요청
                </v-btn>
                <v-dialog v-model="scriptPublishRequestDialog" width="500">
                    <ScriptPublishRequest
                        @closeDialog="scriptPublishRequestDialog = false"
                        @scriptPublishRequest="scriptPublishRequest"
                    ></ScriptPublishRequest>
                </v-dialog>
            </div>
            <BookScriptsOpen @search="search" style="margin-bottom: 10px; background-color: #ffffff;"></BookScriptsOpen>
            ...
            >
                <v-card>
                    <v-toolbar
                        ...
                        height="50px"
                    >
                        <div style="color:white; font-size:17px; font-weight:700;">BookScript 등록</div>
                        <v-spacer></v-spacer>
                        <v-icon
                            color="white"
                            small
            ...
            >
                <v-card>
                    <v-toolbar
                        ...
                        height="50px"
                    >
                        <div style="color:white; font-size:17px; font-weight:700;">BookScript 수정</div>
                        <v-spacer></v-spacer>
                        <v-icon
                            color="white"
                            small
...
export default {
    ...
    data: () => ({
        path: 'bookScripts',
        scriptPublishRequestDialog: false,
    }),
    watch: {
    },
    methods:{
        async scriptPublishRequest(params){
            try{
                var path = "scriptPublishRequest".toLowerCase();
                var temp = await this.repository.invoke(this.selectedRow, path, params)
                // 스넥바 관련 수정 필요
                // this.$EventBus.$emit('show-success','ScriptPublishRequest 성공적으로 처리되었습니다.')
                for(var i = 0; i< this.value.length; i++){
                    if(this.value[i] == this.selectedRow){
                        this.value[i] = temp.data
                    }
                }
                this.scriptPublishRequestDialog = false
            }catch(e){
                console.log(e)
            }
        },
